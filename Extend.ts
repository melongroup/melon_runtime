...
export function array_dosort<T>(arr: Array<T>, ...args) {
    ...
    if (key) {
        return arr.sort((a: any, b: any) => descend ? b[key] - a[key] : a[key] - b[key]);
    } else {
        return arr.sort((a: any, b: any) => descend ? b - a : a - b);
    }
}


export function array_remove<T>(arr: Array<T>,t:T) {
...
export function substitute(str: string, ...args: any[]) {
    let len = args.length;
    if (len > 0) {
        let obj;
        if (len == 1) {
        ...
        if ((obj instanceof Object) && !(obj instanceof RegExp)) {
            return str.replace(/\{(?:%([^{}]+)%)?([^{}]+)\}/g, function (match: string, handler: string, key: string) {
    ...
    return str.toString();//防止生成String对象，ios反射String对象会当成一个NSDictionary处理
}
